...
//Join registering a new client returns pointer to a Client, or Nil, if the name is already taken
...
//AddMsg adding message to queue
func (cr *ChatRoom) AddMsg(msg string) {
	cr.queue <- msg
}

//BroadCast broadcasting all the messages in the queue in one block
...
//NewMsg Client has a new message to broadcast
...
//Send Sending message block to the client
...
func printClientConnInfo() {
	...
	for _, a := range addrs {
		if a.String() != "0.0.0.0" {
			fmt.Println("http://" + a.String() + ":9000/\n")
		}
	}
}
